...
module.exports.sendOtp = async(req,res) => {
  ...
  const data = await User.create({phoneNumber: phoneNumber, otp: otp, name: name});
  res.status(200).json({
    otp: data.otp
  })
}
  ...
  if(!data){
    return res.status(400).json({
     ...
     jwt.sign(
          ...
          (err, token) => {
            if (err) res.status(400).send("Invalid Credentials");
            res.send({ token, Id: data._id, isRegistered: data.isRegisered , role: data.role, msg: "verified! 👍" });
          }
        );
...
module.exports.updateProfile = expressAsyncHandler(async (req, res) => {
  ...
  try {
    const { tradeName, phoneNumber, photo, location,  email, address } =
      req.body;
